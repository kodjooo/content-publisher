...
- `publisher/gs/sheets.py` — клиент Google Sheets: чтение строк по вкладкам, обновление статусов, ссылок и заметок об ошибках (`Notes` / `Moderator Note`).
...
2. **VK**  
   ...
   - Сохранение ссылки в `Post Link`, очистка `Moderator Note`, установка статуса `Published`.
3. **Setka**  
   - Чтение строк со статусом `Revised` на вкладке Setka.  
   - Публикация сообщения в Telegram-канал.  
   - Запись ссылки в `Post Link`, очистка `Moderator Note`, установка статуса `Published`.

При ошибках запись не обновляется; сообщение сохраняется в колонке `Notes` (RSS) или `Moderator Note` (VK/Setka), а лог содержит подробности в формате JSON.
