...
1. **RSS**  
   ...
   - Публикация в VK-сообщество и Telegram, добавление ссылок вида “Читать подробнее >” (VK с попыткой сокращения `vk.cc`; при ошибке сокращения используется исходная ссылка). Перед этим короткий пост очищается от уже добавленного хвоста.  
...
Расписание
----------
- RSS обрабатывается только в 08:00 и 20:00 по московскому времени.  
- VK и Setka выполняются в 18:00 (мск) и только в дни, перечисленные в переменных `VK_PUBLISH_DAYS` и `SETKA_PUBLISH_DAYS`. За один запуск обрабатывается не более одного поста на вкладку.
...
Флаг `RSS_USE_AVERAGE_POST` позволяет переключить публикацию RSS-постов c колонки `Short Post` на `Average Post`. В этом режиме хвост `Источник >` заменяется ссылкой на оригинальную статью из поля `Link`.
