...
## Этап 11. Переход на обновление Excel по ссылкам
Статус: выполнено
- Сохранить текущий вариант обхода через Playwright и пагинацию категорий, но переключить бизнес-поток на мониторинг списка категорий одного сайта и сбор только полей `current_price`, `rating`, `in_stock`.
- Удалить неиспользуемые блоки Google Sheets, state/resume, контентный парсинг карточек и сохранение изображений; оставить HTTP-движок и watchdog/cooldown инфраструктуру.
- Реализовать модуль обновления Excel (`app/excel/updater.py`): сопоставление по `product_url`, обновление только целевых колонок, пропуск отсутствующих ссылок, автосоздание колонок при необходимости.
- Актуализировать `.env`, `.env.example`, `docs/requirements.md`, `docs/architecture.md`, `README.md` под новый сценарий.
- Обновить тесты (`tests/`) под новую архитектуру и проверить прогон `pytest`.
